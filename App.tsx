...
export default function App() {
    ...
    const checkApiKey = async () => {
      try {
        if (window.aistudio && window.aistudio.hasSelectedApiKey) {
          ...
          // If not in AI Studio or API not available, assume we have a key via env vars
          setHasApiKey(true);
        }
      } catch (err) {
  ...
  const handleAnalyze = async () => {
    if (!hasApiKey && window.aistudio) {
      setError('API 키가 필요합니다. 상단의 "API 키 설정" 버튼을 눌러 키를 등록해주세요.');
      return;
    }
    ...
    try {
      ...
      if (errorMessage.includes('API key is missing') || errorMessage.includes('API key not valid')) {
        if (window.aistudio) {
          setHasApiKey(false);
          setError('API 키가 없거나 유효하지 않습니다. 상단의 "API 키 설정" 버튼을 눌러 키를 등록해주세요.');
        } else {
          setError('API 키가 설정되지 않았습니다. Vercel 환경변수(VITE_GEMINI_API_KEY)를 확인해주세요.');
        }
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            {!isCheckingKey && !hasApiKey && window.aistudio && (
              <button
                onClick={handleSelectKey}
                className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-white bg-slate-800 hover:bg-slate-700 rounded-md transition-colors"
              >
